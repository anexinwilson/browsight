 ...
 */
import type { Action, ActResponse, ReadResponse, TabsResponse } from "@browsight/shared";
import type { Bridge, BridgeOptions, BridgeStatus, ReloadResult } from "./bridge.ts";
...
export function createLazyBridge(opts: LazyBridgeOptions): LazyBridge {
  ...
  return {
    ...
    async readActiveTab(url: string | null, mode?: "full" | "main"): Promise<ReadResponse> {
      const bridge = await ensure();
      return await bridge.readActiveTab(url, mode);
    },

    async actActiveTab(req: { ref: string; action: Action; value?: string }): Promise<ActResponse> {
      const bridge = await ensure();
