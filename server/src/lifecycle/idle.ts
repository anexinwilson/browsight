/**
 * Releasing the loopback port after a stretch with nothing to do.
 *
 * Going idle frees the port for another MCP client rather than exiting: the client owns this
 * process's lifetime, and the parent watchdog handles real teardown. The timer is unref'd so it
 * never keeps the process alive by itself, and a zero timeout disables idling entirely.
 */
