...
import { isProcessAlive } from "../../server/src/parent-watchdog.ts";
...
test("stateless bridge: HTTP QUERY fulfills request and immediately closes connection", async () => {
  const port = await getFreePort();
  const token = "unit-stateless-token";
...
test("stateless bridge: fulfills multiple sequential HTTP QUERY requests on same active bridge", async () => {
  const port = await getFreePort();
...
test("stateless bridge: SSE HTTP QUERY fulfills request and immediately terminates stream", async () => {
  const port = await getFreePort();
...
test("stateless bridge: unauthorized requests return 401 and close connection immediately", async () => {
  const port = await getFreePort();
