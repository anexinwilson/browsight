...
test("bridge rejects a request when no extension is connected", async () => {
  // The wait is asserted separately; here we only care about the message once it gives up.
  const bridge = startBridge({ port: getPort(), token: "t", extensionWaitMs: 10 });
  await bridge.ready;
  try {
...
test("request-and-disconnect gateway: client connects via HTTP QUERY, sends request, receives response, connection is closed by server, and daemon accepts second sequential request", async () => {
  const port = getPort();
  const token = "request-disconnect-token";
  const bridge = startBridge({
    port,
    token,
    onTabs: (req) => ({
...
test("request-and-disconnect gateway: handles read.request and act.request with server-side closure", async () => {
  const port = getPort();
  const token = "request-disconnect-read-act-token";
  const bridge = startBridge({
    port,
    token,
    onRead: (req) => ({
...
test("request-and-disconnect gateway: SSE response stream closes upon fulfillment", async () => {
  const port = getPort();
  const token = "request-disconnect-sse-token";
  const bridge = startBridge({
    port,
    token,
    onRead: (req) => ({
...
test("a tool call waits for a cold extension instead of failing outright", async () => {
  const port = getPort();
  const token = "cold-start-token";
  const bridge = startBridge({ port, token });
  await bridge.ready;
  const read = bridge.readActiveTab(null);
  // Swallow the eventual rejection so closing the bridge below does not surface as unhandled.
  const outcome = read.then(
    () => "resolved",
    () => "rejected",
  );
  try {
    // With no extension attached the call must stay pending, not reject on the spot.
    const raced = await Promise.race([
      outcome,
      new Promise((resolve) => setTimeout(() => resolve("pending"), 200)),
    ]);
    assert.strictEqual(raced, "pending", "the call must wait while the extension is cold");
  } finally {
    await bridge.close();
    await outcome;
  }
